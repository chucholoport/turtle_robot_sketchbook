...
static const char NODELETLOAD[] = "nodelet/NodeletLoad";
  ...
  class NodeletLoadRequest : public ros::Msg
  {
    ...
    NodeletLoadRequest():
      name(""),
      type(""),
      remap_source_args_length(0), st_remap_source_args(), remap_source_args(NULL),
      remap_target_args_length(0), st_remap_target_args(), remap_target_args(NULL),
      my_argv_length(0), st_my_argv(), my_argv(NULL),
      bond_id("")
    {
    }
