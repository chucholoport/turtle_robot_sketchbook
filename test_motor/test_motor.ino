 ...
 * test_motor.ino
 *
 * Single DC motor test
 * Arduino Mega 2560 + ROS Noetic (rosserial)
 *
 * This version intentionally uses:
 *   Encoder -> Pin 2 (INT0)
 *
 * to keep the same configuration as Arduino UNO
 * for academic simplicity.
 *
 * ROS:
...
void cmdVelCallback(const geometry_msgs::Twist &msg)
{
  /*
   * For testing:
   * Expect linear.x in range [-1.0 , 1.0]
   * Scale directly to PWM.
   */
